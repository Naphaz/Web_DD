<?php
    ...
    $user_id = $_SESSION['user_id']; // TODO: ก ำหนดตัวแปร user_id จำก session
    $errors = [];
    ...
    // ถำ้ไมม่ สี นิ คำ้ในตะกรำ้
    if (!$items) {
    echo "<h3>ไม่มีสินค้าในตะกร้า</h3></h3>"; // TODO: ขอ้ ควำมแจง้เตอื นวำ่ ไมม่ สี นิ คำ้
    exit;
    }
    // ค ำนวณรำคำรวม
    $total = 0;
    ...
    $errors[] = "กรุณากรอกข้อมูลให้ครบถ้วน"; // TODO: ข ้อควำมแจ้งเตือนกรอกไม่ครบ
    }
    if (empty($errors)) {
    // เริ่ม transaction
...
<div class="col-md-6">
<label for="phone" class="form-label">เบอร์โทรศัพท์</label> ์
